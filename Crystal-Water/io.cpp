  ...
  while (!in.eof()) {
    ...
    if (line.empty()) {
      ...
      mesh.set_ambient(cnt,
          glm::vec3(atof(values[0]), atof(values[1]), atof(values[2])));
    } else if (t == "Kd") {
    ...
    if (line.empty()) {
      ...
      if (t == "v") {
        if (values.size() == 3) {
          ...
          mesh.AddVertex(glm::vec4(atof(values[0]),
              atof(values[1]), atof(values[2]), atof(values[3])));
        }
      } else if (t == "vt") {
        if (values.size() == 2) {
          mesh.AddTextureVertex(glm::vec3(
              atof(values[0]), atof(values[1]), 0.0f));
        ...
        mesh.AddNormalVertex(glm::vec3(v[0], v[1], v[2]));
      } else if (t == "f") {
